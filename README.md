...
> **Status:** v4.0.3 — **Demo hardening (Supabase + snapshot + stałe daty)**. **Następna: 4.1** Auth.
...
- [x] **0.1** Bucket `dart-snapshots` (private, RLS) *(Lovable Faza 0)*
- [x] **0.2** Shell UI PL, routing, wybór palety *(Lovable Faza 0)*
- [x] **0.3** Ingest N01 + parser (endpoint, negative-score) *(Lovable Faza 1.1-1.2)*
- [x] **0.4** Typy TS: `N01Match`, `N01Leg`, `N01Visit` *(Lovable Faza 1.2)*
- [x] **0.5** Detekcja gracza — **stary sposób buggy** (Piotr/Grotkowski/Groteł) *(Lovable Faza 1.3)*
- [x] **0.6** Silnik statystyk: `computeMatchStats`, `computePlayerStats` *(Lovable Faza 1.4)*
- [x] **0.7** Widok `/profile` — kafle stats, lista meczów, filtr zakresu *(Lovable Faza 1.5-1.6)*
- [x] **0.8** Widok `/m/[shareId]` — throw-by-throw *(Lovable Faza 1.6)*
- [x] **0.9** Seed 3 mecze testowe *(Lovable Faza 1.7-1.8)*
- [x] **0.10** Schemat DB (migracje Postgres, RLS deny-by-default) *(Lovable Faza 2.2)*
- [x] **0.11** Persystencja: `saveMatch`, `getMyMatches`, `getMatchByShareId` *(Lovable Faza 2.3)*
- [x] **0.12** Wykres formy (Recharts: 3-dart avg + First 9) *(Lovable Faza 2.4)*
- [x] **0.13** Ostatnie 5 meczów (kafel W/L) *(Lovable Faza 2.5)*
- [x] **0.14** Top 10 najczęstszych podejść *(Lovable Faza 2.6)*
- [x] **0.15** Top 10 najczęstszych zamknięć *(Lovable Faza 5.1)*
- [x] **0.16** Formularz „Dodaj mecz" + walidacja tmid *(Lovable Faza 3.1-3.3)*
- [x] **0.17** Obsługa duplikatów (Nadpisz / Pomiń) *(Lovable Faza 3.2)*
- [x] **0.18** Import hurtowy (textarea, sekwencyjny) *(Lovable Faza 5.2)*
- [x] **0.19** Signed URL do snapshotów (TTL 5 min) *(Lovable Faza 4.1)*
- [x] **0.20** Przycisk „Udostępnij mecz" *(Lovable Faza 4.2)*
- [x] **0.21** Audit-log dostępu *(Lovable Faza 4.3)*
- [x] **0.22** Fix-pack: forma avg, 140+ violet, filtry zakresu (30/90/180/365/all) *(Lovable Faza 4.5)*
...
- [x] **4.0.1.4** **Dataset demo** — 10 meczów zanonimizowanych (obecnie: Supabase + snapshot, patrz **v4.0.3**)
...
**Podmiana postaci demo:** edytuj `demo/demo-persona.ts` → `npm run repolish:demo` → commit snapshot JSON.

**Po v4.0.1:** **4.1** (Auth) — landing **4.5** już działa (`/login` placeholder).

---

### Release v4.0.3 — Demo hardening (Supabase + snapshot) ✅ DONE

> **Cel:** Demo oparte na prawdziwych meczach N01 (zanonimizowanych), statyczne KPI per zakres czasu, daty zawsze „świeże" względem dziś — bez wpływu na profil Piotra Grotkowskiego.

- [x] **4.0.3.1** **Demo w Supabase** — osobny `DEMO_CUSTOMER_ID` (`b0000000-…`), migracja `20260713220000_demo_customer.sql`
- [x] **4.0.3.2** **Import + polish** — `scripts/seed-demo-matches.ts`, `lib/demo-import.ts` (10 linków N01 → anonimizacja tytułów/przeciwników, 3 porażki, sloty godzin)
- [x] **4.0.3.3** **Statyczny snapshot** — `demo/demo-profile-snapshot.json` + `lib/demo-snapshot.ts` (precomputed stats per 30/90/180/365/all, bez liczenia w runtime)
- [x] **4.0.3.4** **Stałe offsety dat** — `DEMO_DAYS_AGO`: 10, 13, 28, 50, 58, 100, 120, 124, 190, 220 dni → **3 / 5 / 8 / 10 / 10** meczów w filtrach
- [x] **4.0.3.5** **Daty live, staty frozen** — `refreshDemoSnapshotDates()` w runtime (daty przesuwają się, KPI z snapshota bez zmian)
- [x] **4.0.3.6** **Skrypty npm** — `seed:demo`, `repolish:demo`, `snapshot:demo` (zastąpiły `build:demo`)
- [x] **4.0.3.7** **Share URL meczu** — `lib/share-url.ts` (`/m/` vs `/demo/m/`)
- [x] **4.0.3.8** **Izolacja od profilu usera** — API `/profile`, `/api/*`, `DEFAULT_CUSTOMER_ID` nietknięte

**Regeneracja demo po zmianach:**

```bash
npm run repolish:demo   # DB + snapshot (zalecane)
npm run snapshot:demo   # tylko JSON z DB
```
...
| 4   | 4.0.1          | **4.0.1.4**  | Anonimizacja → `demo/demo-persona.ts` + snapshot (10 meczów)               |
...
### v4.0.3 — Demo hardening ✅ | Faza 4.1 ⏳ NASTĘPNA


| Element    | Status                                                        |
| ---------- | ------------------------------------------------------------- |
| **v4.0.3** | ✅ Demo Supabase + snapshot KPI + stałe daty (3/5/8/10/10)     |
...
### Najważniejsze zmiany v4.0.3


| Zmiana                              | Impact                                                         |
| ----------------------------------- | -------------------------------------------------------------- |
| `demo/demo-profile-snapshot.json`   | Statyczne KPI per 30/90/180/365/all — zero liczenia w runtime  |
| `DEMO_DAYS_AGO`                     | Stały rozkład meczów w filtrach (3/5/8/10/10)                  |
| `refreshDemoSnapshotDates()`        | Daty zawsze względem dziś; statystyki bez zmian                |
| `npm run repolish:demo`             | Seed/polish DB demo + regeneracja snapshota                    |
| `DEMO_CUSTOMER_ID`                  | Izolacja od profilu Piotra (`DEFAULT_CUSTOMER_ID`)             |
...
Stan v4.0.3 — demo hardening DONE. NASTĘPNA: 4.1 Auth (Google).
Demo: snapshot statyczny + daty live; profil Piotra nietknięty (noindex).
...
| ---------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v4.0.3** | 2026-07-14 | **Demo hardening:** Supabase `DEMO_CUSTOMER_ID`, import 10 meczów N01 + anonimizacja, statyczny snapshot KPI per zakres, stałe offsety dat (3/5/8/10/10), `refreshDemoSnapshotDates`, skrypty `seed:demo`/`repolish:demo`/`snapshot:demo`, share URL meczu. Usunięto `build:demo` + `demo-matches.json`.            |
